...
#' This function can be useful for ability parameter estimation in adaptive testing.
#'
#' @param response A matrix of item responses. For mixed-format tests, a list
...
#' @param item A matrix of item parameters. For mixed-format tests, a list
...
#' @param ability_method The ability parameter estimation method.
#' The available options are Expected \emph{a posteriori} (\code{"EAP"}),
#' Maximum Likelihood Estimates (\code{"MLE"}), and weighted likelihood estimates (\code{"WLE"}).
#' The default is \code{"EAP"}.
#' @param quad A vector of quadrature points for \code{EAP} calculation.
#' If \code{NULL} is passed, it is set as \code{seq(-6, 6, length.out = 121)}.
#' The default is \code{NULL}.
#' @param prior A vector of the prior distribution for \code{EAP} calculation.
#' The length should be the same as \code{quad}.
#' If \code{NULL} is passed, the standard normal distribution is used.
#' The default is \code{NULL}.
#'
#' @return A list containing:
#' \item{theta}{The estimated ability parameter values. If \code{ability_method = "MLE"}
#' and an examinee receives a maximum or minimum score for all items,
#' the function returns \eqn{\pm \infty}{Inf or -Inf}.}
#' \item{theta_se}{The standard errors of ability parameter estimates.
#' It returns standard deviations of posteriors for \code{EAP}s and asymptotic
#' standard errors (i.e., square root of inverse Fisher information) for \code{MLE}.
#' If an examinee receives a maximum or minimum score for all items,
#' the function returns \code{NA} for \code{MLE}.}
#'
#' @author Seewoo Li \email{cu@@yonsei.ac.kr}
#'
#' @export
