...
logLikeli <- function(item, data, theta){
  if(length(theta)!=1){
    ...
    L <- matrix(nrow = nrow(data), ncol = ncol(data))  
    pvector <- t(P(theta = theta,a=item[,1],b=item[,2]))
    lp1 <- log(pvector)
    lp2 <- log(1-pvector)
...
DataGeneration <- function(seed=1, N=2000, nitem=10, prob=0.5, d=1.7, sd_ratio=1, a_l=0.8, a_u=2.5, latent_dist="normal"){
  set.seed(seed)
...
# M1 step
#################################################################################################################
M1step <- function(E, item, max_iter=10, threshold=1e-7, EMiter){
  ...
  for(i in 1:nrow(item)){
    ...
    se[i,] <- sqrt(-c(inv_L2[1,1], inv_L2[2,2])) # asymptotic S.E.
  }
...
# Estimation
#################################################################################################################
Bimodal_MMLE <- function(initialitem, data, range = c(-6,6), q = 12, 
  ...
  if(latent_dist=="Normal"){
    while(iter < max_iter & diff > threshold){
      ...
      M1 <- M1step(E, item=initialitem, EMiter = iter)
      initialitem <- M1[[1]]
